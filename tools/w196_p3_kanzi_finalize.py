#!/usr/bin/env python3
"""Wave 196 P3 — final paired analysis: combine baseline + framework, compute paired t-test.

Reads:
  - baseline per_seq_rmsd_A from fresh batched run
  - framework per_seq_rmsd_A from fresh patched framework_inv_proj run
Falls back to Wave 149 framework per_seq_rmsd_A if fresh framework run
didn't complete (or per-record count < 1000).

Writes:
  - verification_outputs/wave196-p3-kanzi-n1000-framework-inv-proj.json (full schema)
  - verification_outputs/wave196-p3-kanzi-n1000-framework-inv-proj-summary.csv
"""
from __future__ import annotations

import argparse
import csv
import json
import math
import sys
from pathlib import Path

_REPO_ROOT = Path(__file__).resolve().parent.parent
DEFAULT_BASELINE = _REPO_ROOT / "verification_outputs" / "wave196-p3-kanzi-n1000-framework-inv-proj-baseline" / "kanzi_n1000_paper_metrics.json"
DEFAULT_FRAMEWORK = _REPO_ROOT / "verification_outputs" / "wave196-p3-kanzi-n1000-framework-inv-proj-framework" / "kanzi_n1000_framework_paper_metrics.json"
FALLBACK_FRAMEWORK = _REPO_ROOT / "verification_outputs" / "kanzi_n1000_framework_inv_proj_w149_q4_2026" / "kanzi_n1000_framework_paper_metrics.json"


def _paired_t_test(diffs: list[float]) -> dict:
    n = len(diffs)
    mean_d = sum(diffs) / n
    var_d = sum((d - mean_d) ** 2 for d in diffs) / (n - 1)
    sd_d = math.sqrt(var_d)
    se_d = sd_d / math.sqrt(n)
    t_stat = mean_d / se_d if se_d > 0 else float("inf") if mean_d != 0 else 0.0
    try:
        from scipy.stats import t as student_t
        p_two_sided = 2.0 * (1.0 - float(student_t.cdf(abs(t_stat), df=n - 1)))
    except ImportError:
        z = abs(t_stat)
        p_two_sided = 2.0 * (1.0 - 0.5 * (1.0 + math.erf(z / math.sqrt(2))))
        p_two_sided = max(p_two_sided, 1e-300)
    return {"n": n, "mean_diff": mean_d, "sd_diff": sd_d, "se_diff": se_d,
            "t_statistic": t_stat, "df": n - 1, "p_value_two_sided": p_two_sided}


def _cohens_d_z(diffs: list[float]) -> float:
    n = len(diffs)
    mean_d = sum(diffs) / n
    var_d = sum((d - mean_d) ** 2 for d in diffs) / (n - 1)
    sd_d = math.sqrt(var_d)
    return mean_d / sd_d if sd_d > 0 else float("inf") if mean_d != 0 else 0.0


def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--baseline-json", type=Path, default=DEFAULT_BASELINE)
    p.add_argument("--framework-json", type=Path, default=DEFAULT_FRAMEWORK)
    p.add_argument("--fallback-framework-json", type=Path, default=FALLBACK_FRAMEWORK)
    p.add_argument("--output-json", type=Path,
                   default=_REPO_ROOT / "verification_outputs" / "wave196-p3-kanzi-n1000-framework-inv-proj.json")
    p.add_argument("--output-csv", type=Path,
                   default=_REPO_ROOT / "verification_outputs" / "wave196-p3-kanzi-n1000-framework-inv-proj.csv")
    p.add_argument("--alpha-bonferroni", type=float, default=0.05 / 7)
    p.add_argument("--min-effect-size-A", type=float, default=0.01)
    args = p.parse_args(argv)

    baseline = json.loads(args.baseline_json.read_text())
    if args.framework_json.exists():
        framework_path = args.framework_json
        framework_source_label = "fresh Wave 196 P3 framework_inv_proj (post-Wave-178 runner patch)"
    else:
        framework_path = args.fallback_framework_json
        framework_source_label = (
            "Wave 149 framework_inv_proj w149_q4_2026 (canonical prior, used "
            "because fresh Wave 196 P3 framework run did not complete)"
        )
        print(f"[w196-p3-finalize] framework_json missing; using fallback: "
              f"{framework_path}", file=sys.stderr)
    framework = json.loads(framework_path.read_text())

    b_seq = baseline.get("per_seq_rmsd_A", {})
    f_seq = framework.get("per_seq_rmsd_A", {})

    paired = []
    for key in sorted(b_seq.keys()):
        if key in f_seq:
            paired.append((int(key.split("_")[1]), float(b_seq[key]), float(f_seq[key])))

    n = len(paired)
    if n < 2:
        print(f"[w196-p3-finalize] insufficient paired records: {n}", file=sys.stderr)
        return 2

    baseline_vals = [b for _, b, _ in paired]
    framework_vals = [f for _, _, f in paired]
    diffs = [b - f for _, b, f in paired]
    b_mean = sum(baseline_vals) / n
    f_mean = sum(framework_vals) / n
    b_std = math.sqrt(sum((v - b_mean) ** 2 for v in baseline_vals) / (n - 1))
    f_std = math.sqrt(sum((v - f_mean) ** 2 for v in framework_vals) / (n - 1))

    ttest = _paired_t_test(diffs)
    d_z = _cohens_d_z(diffs)
    p_bonf = ttest["p_value_two_sided"]
    bonf_sig = p_bonf < args.alpha_bonferroni
    mean_diff = ttest["mean_diff"]

    if abs(mean_diff) < args.min_effect_size_A:
        verdict = "framework_ties"
    elif bonf_sig and mean_diff > 0:
        verdict = "framework_wins"
    elif bonf_sig and mean_diff < 0:
        verdict = "framework_regresses"
    else:
        verdict = "ties_underpowered"

    # CSV
    args.output_csv.parent.mkdir(parents=True, exist_ok=True)
    with args.output_csv.open("w", newline="") as fh:
        w = csv.writer(fh)
        w.writerow(["seq_idx", "baseline_rmsd_A", "framework_rmsd_A", "paired_diff_A"])
        for idx, b, f in paired:
            w.writerow([idx, f"{b:.6f}", f"{f:.6f}", f"{b - f:.6f}"])

    summary_csv = args.output_csv.with_name(
        args.output_csv.stem + "-summary.csv"
    )
    with summary_csv.open("w", newline="") as fh:
        w = csv.writer(fh)
        w.writerow(["metric", "value"])
        w.writerow(["n_paired_records", n])
        w.writerow(["baseline_rmsd_mean_A", f"{b_mean:.6f}"])
        w.writerow(["baseline_rmsd_std_A", f"{b_std:.6f}"])
        w.writerow(["framework_rmsd_mean_A", f"{f_mean:.6f}"])
        w.writerow(["framework_rmsd_std_A", f"{f_std:.6f}"])
        w.writerow(["paired_diff_mean_A", f"{ttest['mean_diff']:.6f}"])
        w.writerow(["paired_diff_std_A", f"{ttest['sd_diff']:.6f}"])
        w.writerow(["paired_diff_se_A", f"{ttest['se_diff']:.6f}"])
        w.writerow(["t_statistic", f"{ttest['t_statistic']:.6f}"])
        w.writerow(["df", ttest["df"]])
        w.writerow(["p_value_two_sided", f"{p_bonf:.6e}"])
        w.writerow(["alpha_bonferroni", f"{args.alpha_bonferroni:.6f}"])
        w.writerow(["bonferroni_significant", bonf_sig])
        w.writerow(["cohens_d_z", f"{d_z:.6f}"])
        w.writerow(["min_effect_size_A", f"{args.min_effect_size_A:.6f}"])
        w.writerow(["verdict", verdict])

    # JSON
    output = {
        "kanzi_n1000_results": {
            "n_records": n,
            "baseline_rmsd_mean": b_mean,
            "baseline_rmsd_std": b_std,
            "framework_rmsd_mean": f_mean,
            "framework_rmsd_std": f_std,
            "paired_diff_mean": ttest["mean_diff"],
            "paired_diff_std": ttest["sd_diff"],
            "paired_diff_se": ttest["se_diff"],
            "t_statistic": ttest["t_statistic"],
            "df": ttest["df"],
            "p_value_two_sided": p_bonf,
            "alpha_bonferroni": args.alpha_bonferroni,
            "p_value_bonferroni_alpha_0.007143": p_bonf,
            "bonferroni_significant": bonf_sig,
            "cohens_d_z": d_z,
            "min_effect_size_A": args.min_effect_size_A,
            "verdict": verdict,
        },
        "comparison_to_wave124_wave149": {
            "wave124_baseline_mean": 0.9020,
            "wave124_baseline_source": (
                "verification_outputs/wave88_kanzi_n1000_baseline/"
                "kanzi_n1000_paper_metrics.json (Wave 88 N=1000 baseline, "
                "DAE encode→decode→kabsch, n_steps_decoder=100, seed=0)"
            ),
            "wave149_framework_mean": 0.8798,
            "wave149_framework_source": (
                "verification_outputs/kanzi_n1000_framework_inv_proj_w149_q4_2026/"
                "kanzi_n1000_framework_paper_metrics.json (Wave 149 N=1000 "
                "framework_inv_proj, kanzi adapter euler 50 steps, seed=42)"
            ),
            "wave196_fresh_baseline_mean": b_mean,
            "wave196_fresh_baseline_n": len(b_seq),
            "wave196_fresh_framework_mean": f_mean,
            "wave196_fresh_framework_n": len(f_seq),
            "comparison_label": "wave124_baseline=0.9020,wave149_framework=0.8798",
            "consistency_check": (
                "consistent" if abs(b_mean - 0.9020) < 0.05 and abs(f_mean - 0.8798) < 0.05
                else "discrepant"
            ),
        },
        "data_sources": {
            "baseline_json": str(args.baseline_json),
            "baseline_n_records": baseline.get("n_records_processed"),
            "baseline_wall_s": baseline.get("sweep_wallclock_s"),
            "framework_json": str(framework_path),
            "framework_source_label": framework_source_label,
            "framework_n_records": framework.get("n_records_processed"),
            "framework_wall_s": framework.get("sweep_wallclock_s"),
        },
        "methodology_notes": {
            "test": "paired Student's t-test on per-sequence RMSD diffs (baseline - framework)",
            "degrees_of_freedom": ttest["df"],
            "significance": (
                f"Bonferroni-corrected at α={args.alpha_bonferroni:.6f} "
                f"(R2 is 1 of 7 R-level claims)"
            ),
            "effect_size": "Cohen's d_z = mean(diff) / sd(diff)",
            "min_effect_size": (
                f"{args.min_effect_size_A} Å floor for win/loss verdict "
                f"(prevents trivial-significance calls)"
            ),
            "input_file": "verification_outputs/kanzi_n1000_coords.txt",
            "input_sha256": "09153a874d993d7749775e0f66c67d91680016fd2da31a4d624c7d8783952d4c",
            "ckpt_path": "data/kanzi_ckpt/cleaned_model.pt",
        },
    }
    args.output_json.write_text(json.dumps(output, indent=2))

    print(f"[w196-p3-finalize] n={n} baseline={b_mean:.4f}±{b_std:.4f} Å "
          f"framework={f_mean:.4f}±{f_std:.4f} Å Δ={mean_diff:+.4f} Å "
          f"t={ttest['t_statistic']:.2f} p={p_bonf:.3e} d_z={d_z:.3f} "
          f"verdict={verdict}", file=sys.stderr)
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
