...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--input", type=Path, required=False,
                   help=("Wave 80 extractor output (one record per line). "
                         ...
                         "does not consume the input file — it constructs "
                         "the adapter and runs 1 synthetic record through "
                         "the full protocol)."))
    ...
    p.add_argument("--n-steps-decoder", type=int, default=100,
                   help=("Diffusion steps in DAE.decode inside the bridge "
                         "(Wave 111 F-A003 closure; replaces the prior "
                         "hardcoded nfe_steps=100). Default 100 matches the "
                         "Wave 87 baseline contract."))
    ...
    # exit) on shape mismatch. Industry pattern: Differential
    # Transformer ``sanity_test.py`` dry-runs 1 record before the
    # full sweep.
    p.add_argument("--dry-run", action="store_true",
                   help=("Wave 113.A.5 Fix 2: dry-run 1 record through "
                         ...
                         "mismatch. Pairs with --config to exercise "
                         "the profile-driven path. Pairing with "
                         "--no-config returns exit 78 (SKIP, "
                         "env-no-config gate)."))
    # Wave 113.A.5 Fix 2 — explicit opt-out flag for the config-driven
    # path. When combined with --dry-run, the driver skips the
    # shape-contract probe with exit 78 (matches the Autotools
    # sysexits.h convention). Use this in environments that should
    # NOT exercise the contract probe (e.g. CI smoke tests that
    # explicitly verify the gate is wired).
    ...
    # record). Without this gate, the legacy "no --input" call site
    # would silently skip the input parse and crash later in
    # ``run_kanzi_sweep`` with a confusing NoneType traceback.
