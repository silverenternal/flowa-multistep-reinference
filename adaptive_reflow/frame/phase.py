    ...
    *,
    ...
    lineage_digest = ArtifactHash(
        __import__("hashlib").sha256(
            f"adaptive_reflow.phase_state.run_seed:{run_id_seed}".encode()
        ).hexdigest()
    )
...
def validate_phase_state_transition(
    ...
    * ``phase_state_digest`` on ``new`` matches its deterministic
    ...
    if int(new.outer_cycle_id) == int(prev.outer_cycle_id) and int(new.round_in_cycle) < int(prev.round_in_cycle):
        errors.append(
