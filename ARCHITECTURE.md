...
**Status:** refactor complete. 430 tests pass (362 previous + 68 new
universal/molecular split tests). Top-level flat modules and the
in-tree `.py` files in the repo root are **stale re-export shims** that will
be removed in a follow-up pass.
...
### 5.1 Reference implementation — `ToyLinearAdapter`

The smallest working adapter is shipped at
`adaptive_reflow/adapters/toy_linear.py` (172 lines). It implements the
eight-method `FlowMatchingODEAdapter` Protocol with one continuous
channel, a deterministic linear ODE step, and **no** restart / condition
support. It exists as a sanity-check target for the universal engine and
as a copy-this-skeleton template for new adapter authors.

Tests at `tests/test_adapters/test_toy_linear.py` cover:
...
- Fail-closed for unadvertised capabilities: `compose_condition` and
  `apply_restart_distribution` raise `CapabilityMissingError` because
  `has_condition_injection=False` and `has_restart_boundary=False`
...
5. **`tests/test_universal/test_adapter_universality.py`** — a synthetic
   non-molecular adapter (graph-flow / latent-flow hybrid) satisfies the
   universal `FlowMatchingODEAdapter` Protocol without importing
   `adaptive_reflow.molecular`.
...
8. **Full test suite (475 tests)** — round-trip + capability handshake +
   ...
   (Hypothesis) + adversarial hostile-case fixtures + end-to-end round
   walks + doc-drift scanner self-test + golden replay + kernel
   benchmarks + memory-footprint probes.
...
| Adversarial | One test per hostile case in DESIGN_BOUNDARY.md §3; fail-closed | `tests/test_adversarial/` |
...
| `mutation-nightly.yml` | nightly 03:00 UTC + manual | `bash tools/mutate/run_mutmut.sh`; `mutmut-report` artifact uploaded |
...
either (a) an equivalent mutant or (b) a missing test; the latter is
filed as a follow-up and blocks the PR that introduced it.
