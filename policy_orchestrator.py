...
from .channel_rule import compute_channel_decision
from .cosine_schedule import CosineScheduleSampler
from .envelope_manifest import classify_endpoint
from .operation_composition import build_default_composition_contract
    ...
    * delegates writer arbitration to a :class:`WriterArbitrator`
    ...
    def __init__(
        ...
        *,
        ...
        authority_contract: Optional[RestartPolicyAuthorityContract] = None,
        operation_contract: Optional[OperationCompositionContract] = None,
        arbitrator: Optional[WriterArbitrator] = None,
        schedule_sampler: Optional[CosineScheduleSampler] = None,
    ) -> None:
    ...
    @property
    def last_ledger(self) -> Optional[DynamicRestartTransferLedger]:
    ...
    def evaluate_bundle(
        ...
        6. Build a :class:`DynamicRestartTransferLedger` with
        ...
        7. Cache the row alongside the derived freeze-admission mask
        ...
        for channel in _CANONICAL_CHANNELS:
            ...
            beta_by_channel[channel] = decision.beta
            raw_fraction_by_channel[channel] = decision.scheduled_cap
            bounded_fraction_by_channel[channel] = decision.bounded_target_fraction
            ...
            non_zero_beta = (
                bool(decision.gate)
                and _safe_factor_value(decision.beta, default=0.0) > 0.0
            )
            freeze_admission_by_channel[channel] = bool(non_zero_beta)

        # --- 5. assemble the ledger row ---
        ...
        # --- 7. surface schedule_sample via the schedule_sampler cache ---
    ...
    def build_final_policy(
        ...
        record: Optional[_LedgerRecord] = None
        ...
        schedule_sample: Optional[CosineScheduleSample] = self._schedule_sampler.last_sample
    ...
    def reset_cycle(self) -> None:
        ...
        but the ledger cache and the cached schedule sample are wiped.
        The envelope manifest, schedule config, contracts and
        arbitrator are not reset — they are bound at construction.
        """
...
def _empty_phase_state_for_eval() -> PhaseState:
    ...
    production.
    """
    from .restart_memory_types import make_default_phase_state
