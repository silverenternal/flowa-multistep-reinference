...
## CLM-076: Wave 244-260 — Defensive patch to vendored upstream `data/FlowMol3/repo/flowmol/analysis/metrics.py` REVERTED 2026-09-22 per user academic-integrity directive. Original upstream code (commit 77cae22) is restored. Wave 242 seed 44 retry v5 (and prior v1-v4) all FAIL with the original metrics.py bugs (`rdkit.Chem.Mol` objects lack `.num_atoms` / `.atom_types` / `.valencies` / `.atom_charges` / `.fake_atoms` / `.rdkit_mol` / `.build_molecule`). R3 verdict falls back to Wave 87 seed 42 N=1000 framework_WINS + Wave 208 P2 per-record framework_WINS (d_z=-0.285 N=200 Bonferroni-significant) + Wave 216 P1 projected N=1000 framework_WINS. 3-seed pooled analysis flagged as **BLOCKED at vendor level** — Wave 109.C §5 framework-side code fix required (cannot condition on local-only vendored patches). Protocol-mismatch confound disclosure + DGL 2.4.0 batched-path bug root-cause disclosure preserved via §15.99 / §15.101 / §15.102 + paper Limitations paragraph. Background seed-44 retry (PIDs 3492207 wrapper + 3492209 sweep) terminated before revert per user directive. {#CLM-076}

Background: Wave 87 sweep default NFE_BATCH=100 (batched DGL path) had failed with `DGLError: Expect number of features to match number of nodes (len(u)). Got N and N*10 instead`; NFE_BATCH ∈ {2, 3, 4, 5, 10, 100} all fail; only NFE_BATCH=1 (single_mol path) ran without the DGL error — but produced `Mol` objects upstream analytics could not consume. Wave 244 P5 / 245 P1 / 258 P1 / 259 P1 added defensive `try/except` + `getattr(..., None)` + `del rdmol`/`del df_pb` guards to metrics.py (commit `590ce42` HEAD before revert) to bridge the gap. Per 2026-09-22 user directive "所有对于别人官方仓库里做的所有更改都要撤回" (revert all changes to others' official repos), all those local patches were reverted at commit `4af95da`; restored metrics.py md5 `20a3adbcbf09e631ae5519f5dbf4f117` matches upstream hash. Wave 242 P1 wrapper script `scripts/wave242_p1_flowmol3_rescue_single_mol.py` retained (does not modify framework or metrics.py), but cannot produce a successful seed 44 arm against upstream metrics.py — the 3-seed pooled analysis must wait for the Wave 109.C §5 framework-side fix.

## CLM-077: Wave 260 P1 — Revert of all Wave 244/245/258/259 defensive patches to vendored upstream `data/FlowMol3/repo/flowmol/analysis/metrics.py` per user directive "所有对于别人官方仓库里做的所有更改都要撤回"; vendored file is byte-identical to upstream (md5 `20a3adbcbf09e631ae5519f5dbf4f117`); background seed-44 retry killed before revert; all gates verified GREEN (D.4 30/30 PASS, mkdocs strict 0 warnings, claims consistency no drift) {#CLM-077}

Per user 2026-09-22 directive ("所有对于别人官方仓库里做的所有更改都要撤回，之前后台启动的那个重跑也要kill，严肃一点，按照最干净的方法来"), all four rounds of Wave 244/245/258/259 local defensive patches applied to the vendored upstream file `data/FlowMol3/repo/flowmol/analysis/metrics.py` were reverted at commit `4af95da` (Wave 260 P1, 2026-09-22). Restoration verified by `md5sum` against the upstream hash recorded in Wave 220 vendoring metadata: `20a3adbcbf09e631ae5519f5dbf4f117` (byte-identical to upstream). Diff vs pre-revert HEAD (`590ce42` Wave 259 P1): 15 insertions / 76 deletions, removing the `try/except AttributeError`, `getattr(..., None)`, and `del rdmol`/`del df_pb` defensive wrappers and restoring the upstream direct-attribute access patterns at lines 111 / 349-358 / 363 / 394-401 (and the Wave 245 P1 line 111 3-tier fallback).
...
Gates verified GREEN at `4af95da`: (1) D.4 byte-stable `tests/test_d4_regression_vectors.py` 30/30 PASS; (2) `mkdocs build --strict` 0 warnings; (3) `tools/check_claims_consistency.py` "**No drift detected.**" Audit trail at `docs/audit/wave260-p1-gates-verify.md`; companion wording-reconciliation audit at `docs/audit/wave260-p2-update-claims.md`.

Hard-rule compliance: framework source code untouched; vendored upstream code restored byte-identical to upstream; background tasks none active. The revert is the **only** allowed local change to `data/FlowMol3/repo/flowmol/analysis/metrics.py` between upstream commit and HEAD, and it operates strictly in the delete-direction (removing local-only defensive wrappers). No new content was introduced into vendored upstream code; no upstream file is part of any future commit in this revert lineage. 3-seed FlowMol3 pooled analysis is now **BLOCKED at vendor level** (CLM-076) until Wave 109.C §5 framework-side fix lands; R3 verdict falls back to the Wave 87 seed 42 N=1000 framework_WINS lineage (CLM-076 second paragraph + Wave 208 P2 + Wave 216 P1).
