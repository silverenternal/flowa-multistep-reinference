...
## CLM-076: Wave 242 P1 (in flight) — FlowMol3 3-seed rescue via single_mol path workaround for DGL 2.4.0+cu124 batched-path DGLError {#CLM-076}

Pre-Wave 242: Wave 87 sweep (`tools/wave87_n1000_sweep.py`) default NFE_BATCH=100 (batched DGL path) failed with `DGLError: Expect number of features to match number of nodes (len(u)). Got N and N*10 instead` at every batch (Wave 240 P2 + Wave 242 deep-diagnosis confirmed). NFE_BATCH ∈ {2, 3, 4, 5, 10, 100} all fail; only NFE_BATCH=1 (single_mol path) works. Wave 242 P1 implements wrapper script `scripts/wave242_p1_flowmol3_rescue_single_mol.py` that invokes the Wave 87 sweep with hardcoded `--nfe-batch 1 --n-total 200 --nfe 250 --device cuda:0` (single_mol path, N=200 NFE=250 per seed; ~33 min/arm × 4 arms = ~2.2 GPU-h total). Does NOT modify framework source code or Wave 87 sweep internals (D.4 30/30 PASS preserved); only adds a wrapper that calls `wave87_n1000_sweep.py` with strict args.

Wave 242 P1 status (2026-09-21 22:17 CST): seed 43 baseline + framework COMPLETED (200/200 mols each arm; framework fg_dev=0.7361 vs baseline computed fg_dev; n_errors=0; n_dropped=0; 1.0000 validity); seed 44 baseline + framework RUNNING (PIDs 3492207 wrapper + 3492209 sweep; ~30 min remaining). Wave 243 P2-P4 (queued) will produce the final per-seed d_z + 3-seed pooled verdict with REAL data and re-update the paper abstract + section-2-method + cover letter. The §15.99 / §15.101 / §15.102 disclosures + paper Limitations paragraph preserve the **protocol-mismatch confound** (seed 42 N=1000 batched vs seeds 43/44 N=200 single_mol) and the **DGL 2.4.0 batched-path bug** root-cause disclosure regardless of the eventual verdict direction.
