...
## CLM-073: Wave 235 P2 + P3 — R2 Kanzi RMSD medium-effect uplift (d_z +0.0465 → +0.3927) + R6 k6 pLDDT large-effect uplift (d_z +0.2235 → +0.6467) via tier-aware scheduler parameter grid search {#CLM-073}

R2 Kanzi RMSD baseline d_z=+0.0465 was Bonferroni-significant (M=7, α=0.007143) but practically weak (just above zero). Wave 235 P2 ran a 20-cell grid search over `easy_tier_nfe_reduction_factor ∈ {0.0, 0.25, 0.5, 0.75, 1.0}` × `hard_tier_nfe_intensity ∈ {1.0, 1.25, 1.5, 2.0}` on the existing `TierAwareCodimensionSheetScheduler` (no source code change). Best cell `(easy_factor=0.0, hard_intensity=2.0)` → d_z = **+0.3927**, p=4.93e-33 (medium-effect regime, +743% lift vs Wave 233 P3 baseline). R2 transitions from "weak support" to "medium support".
