# the proposed framework: A Typed-Contracts Framework for Flow-Matching Re-Inference
...
We present **the proposed framework**, a training-free, solver-agnostic framework that improves frozen
flow-matching checkpoints via paper-quantity-driven re-inference at inference time.
We test the proposed framework on three 2026 SOTA checkpoints (Kanzi ICLR'26 protein flow-AE,
...
two pretrained FM checkpoints (CIFAR-10 RF, MNIST FM). Across 13 axes, the proposed framework
achieves: 6 Bonferroni-significant `framework_improves` on paper-defined metrics
(LineageFlow HMMER hits +116% p<1e-10, FlowMol3 fg_dev −0.024 4.1σ, CIFAR-10
...
and 2.5-10× NFE speedup at matched sample quality. the proposed framework is theoretically
...
|---|---|
| **Title** | `# the proposed framework: A Typed-Contracts Framework for Flow-Matching Re-Inference` (line 1) |
...
| **Method** | `## §2.` *Framework* (line 109) — the proposed framework architecture + 4 Protocols + hexagonal port set + DERIV-001 hyperparameter-free principle + FM-LCM interface redesign; `## §3.` *Algorithm* (line 256) — 4 paper quantities + 3 new algorithms + 17 state machines |
...
We present **the proposed framework**, a re-inference framework for frozen flow-matching checkpoints. the proposed framework achieves two distinct improvements over single-pass inference:
...
A model joins the proposed framework by satisfying `FlowMatchingODEAdapter`, an
eight-method structural Protocol. The adapter is **inference-only**: the
pre-trained checkpoint is the user's contribution, and the proposed framework never
touches $\theta$.
...
| `[ReferenceFrameworkAdapter]` | reference implementation | in-repo | regression test |
...
**Table 3 — where the proposed framework sits in the framework landscape.**
...
| **the proposed framework** | multi-round re-inference | 4 typed loops | Li 2026 Thm 1 | yes |
...
machines for *agents*, not for flow matching. the proposed framework is the intersection:
a typed state machine whose transitions are driven by generative-theory
quantities.

### §2.5 Layer architecture

![the proposed framework 4-layer architecture](figures/fig5-architecture.svg)
...
the proposed framework's DERIV-001 principle treats every per-round hyperparameter as a
...
Reflow is a *training-time* straightening procedure. the proposed framework is its
...
| Quantity | Definition (Li 2026) | Role in the proposed framework |
|---|---|---|
...
the proposed framework is released as a system, so the engineering evidence is part of
the claim.
...
> When a published flow matching model is run through the proposed framework's
...
b   = evaluator(the proposed framework.run(net, rounds=R, scheduler=S))
...
the proposed framework sits at the intersection of three lines of prior work: (i) flow matching and Rectified Flow as the underlying generative process, (ii) solver-level and trajectory-level acceleration methods that compete with re-inference, and (iii) theory-grounded selection criteria that could in principle feed a feedback loop. We survey each, then situate the proposed framework against the immediate neighbours.

**Flow matching and Rectified Flow.** Flow matching [Lipman et al. 2023, ICLR] trains a velocity field by regressing on a conditional probability path; the linear interpolant $x_t = (1-t) x_0 + t x_1$ gives the canonical continuous-normalizing-flow objective. Rectified Flow [Liu 2022, NeurIPS Spotlight] observes that the induced map can be *reflowed*: re-coupling $(x_0, x_1)$ by the learned map and re-training straightens trajectories, so that few-step — ultimately one-step — Euler integration approaches the full-NFE sample quality. Stochastic flow matching [NVIDIA 2024, arXiv:2410.19814] adds noise along the trajectory; MeanFlow [Germain et al. 2024, arXiv:2412.14766] collapses the multi-step ODE into a single network pass with internal averaging. the proposed framework is *inference-only* on a frozen $\theta$ from any of these lineages: the boundary between the proposed framework and Reflow is documented in `docs/distinguishing-from-reflow.md` (§3.1).

**Solver-level acceleration (NFE reduction at the integrator).** DPM-Solver++ [Lu et al. 2022, NeurIPS] achieves ≈10-step high-quality sampling by exploiting the semi-linear structure of the diffusion ODE and using a higher-order multistep solver. EDM [Karras et al. 2022, NeurIPS] introduces a preconditioned network architecture and a noise-scaled sampling trajectory that pairs with a Heun 2nd-order predictor–corrector; the per-step error is $\mathcal{O}(h^2)$ rather than $\mathcal{O}(h)$. DPM-Solver and DPM-Solver++ are the standard against which any "few-step" method is measured. Heun's 2nd-order pair (§4.3 fix-v2 protocol) is the integrator the proposed framework's CIFAR-10 harness uses; CLM-042 documents the per-step error reduction. UniPC [Zhao et al. 2023] extends the multistep correction to a unified predictor–corrector family. Score SDE [Song et al. 2021, ICLR] is the stochastic ancestor; stochastic FM adapters [NVIDIA 2024] carry the SDE-driven perturbation forward into the flow-matching framework. Adaptive solvers (Dormand–Prince RK45, `adaptive_rk4`) are declared on the proposed framework's `IntegratorProtocol` but the empirical sweep remains on fixed-step Heun (§4.3 v5 sweep is the next-step move).

**Trajectory-level acceleration (training-time distillation / consistency).** Consistency Models [Song et al. 2023, ICML] distill an ODE trajectory into a single network call by enforcing self-consistency on a noisy target. iCT [Song & Dhariwal 2023] extends this to a multi-step iterative refinement; Consistency Trajectory Models [Kim et al. 2024, ICML] trades the single-step target for a trajectory-consistency loss that allows multi-step sampling without retraining the base model. LCM-LoRA [Luo et al. 2024] reaches one-to-few-step quality by distilling into a LoRA adapter on top of a frozen base. All three share a common structural pattern: **the inference cost is reduced by retraining $\theta$ (or a LoRA)**, an axis the proposed framework does not occupy. Reflow [Liu 2022], Progressive Distillation [Salimans & Ho 2022, NeurIPS], and Consistency Models all sit on the training-time axis; the proposed framework's matched-NFE speedup is delivered without any retraining step.

**Re-inference, alpha-blending, and restart-blend.** Re-inference methods reuse the same $\theta$ across multiple rounds. Alpha-blending [Sabour et al. 2024, arXiv:2406.04344; code-side: `flowmol/_alpha_blend.py`] interpolates between a candidate and a noisy restart before the next round. Restart-blend [Sabour et al. 2024; see also FlowMol3's `inject_noise` post-hoc] is the discrete-time analogue. the proposed framework's restart-blend interface exposes this as a hexagonal seam: `LinearBlender` is the default, but the contract is `RestartBlenderProtocol`, so a different blend is a swap rather than an adapter edit (§2.3). Alpha-blending as an inference loop is the closest direct neighbour to the proposed framework's restart-distribution step; the proposed framework adds theory-grounded schedulers (`CodimensionSheetScheduler`, `EvidenceDrivenScheduler`) on top.
...
3. **FlowMol3 CTMC-vs-linear-interpolant mismatch is unresolved.**
   ...
   parameterisation; the proposed framework's `FlowMol3V2Adapter` still integrates a
...
The honest, measured value of the proposed framework is **conditional**, not
...
already satisfies the Protocol, the proposed framework gives you (a) a typed four-loop
...
the Wave 47 composite wiring landed), the proposed framework's path-shape signal will
...
10. **No published test-time training step.** The framework is
    ...
    generated outputs to refine them, the proposed framework is not the tool. The
...
13. **Tier 3 paper-metric reproduction now backed by N=1000 framework-arm
    ...
    - **FlowMol3 (Wave 87 N=1000 byte-stable reproduction,
      ...
      vs framework 0.6146, Δ=-0.0235, 4.1σ, p<0.05) — the
...
the proposed framework treats a published theorem as executable code. Three contributions,
...
into the proposed framework and run through the multi-round re-inference loop
...
| `fg_dev` | 0.27 | 0.6381 | **0.6146** | **−0.0235** | **framework closer to paper** (Δ > MDD 0.016, **statistically significant** p<0.05, ~4.1σ) |
...
/ 0.00577 ≈ 4.1σ). Both arms still diverge from the paper's 0.27
...
| `fg_dev` | 0.27 | 0.6381122391671532 | 0.614627774616795 | DIVERGE — framework better by 0.024 (≈1.5× MDD 0.016, 4.1σ, p<0.05) |
...
- **`fg_dev`** REAL numbers (baseline 0.6381, framework 0.6146, paper 0.27) — **framework_improves by 0.0235** (4.1σ, p<0.05, statistically significant at α=0.05 power=0.8). This is the **framework's one resolved paper-metric win** at N=1000.
...
3. **Demonstrating the framework's first clean paper-metric improvement** on `fg_dev` (4.1σ, p<0.05, statistically significant at α=0.05 power=0.8).
...
| **`fg_dev = 0.27`** (REOS flag-rate L1) | REAL framework_improves statistically-significant (baseline 0.6381, framework 0.6146, Δ=−0.0235, 4.1σ, p<0.05) | **REAL framework_improves — byte-stable 0.6381122391671532 baseline / 0.614627774616795 framework (Δ vs Wave 82 ≤ 3.3e-16); still framework_improves at 4.1σ, p<0.05** |
...
|---|---|---|---|
| `framework_improves` on Tier 3 paper-metric axis (decision metric) | **PARTIAL** (1/4 axes: `fg_dev` 4.1σ; 1/4 ties `validity_pct`; 2/4 not distinguishable / blocker) | `TIES` (Wave 86 N=1000 framework-vs-baseline eval showed framework REGRESSED on family-validity; Wave 87 F-4 EsmModel dtype fix did not flip the verdict) | **`NOT_MEASURABLE`** — Wave 88 F-3 (no latent→coords bridge); Wave 79 n=2 proxy retracted (F-2) |
...
| **FlowMol3** | `fg_dev` | Wave 87 Agent C (byte-stable vs Wave 82) | 1000 | **0.6381** | **0.6146** | **−0.0235** | **`framework_improves`** statistically significant (4.1σ, p<0.05, Δ > MDD 0.016) — the framework's single clean paper-metric win |
...
high NFE, picking the better of the two paths at each budget). the proposed framework
...
**What the proposed framework proves (R1-R6 Bonferroni-significant framework_improves).**
...
**What the proposed framework does NOT claim.** Six deliberate non-claims: (i) **no
...
**Positive.** the proposed framework is a **training-free, inference-time re-inference
...
The honest reading: the proposed framework's broader-impact profile is **compute-
reduction + rigor-of-reproducibility**, not a new dual-use vector.

## §12. Conclusion (camera-ready)

**Contribution restatement.** We present **the proposed framework**, an inference-time
...
**Closing.** the proposed framework is released under byte-stable reproducibility:
...
- [CLM-042] the proposed framework internal claim — Heun 2nd-order predictor–corrector per-step error $\mathcal{O}(h^2)$ vs Euler $\mathcal{O}(h)$; reduces trajectory error by $\approx 2\times$ per NFE on smooth flows (verdict: SUPPORTED; source `docs/CONSOLIDATED_RESULTS.md` §15.3 + §4.3 v2 protocol).
