# Wave 211 P5 — §Methods: Experiment Setup Documentation

**Date:** 2026-09-21
**Agent:** Wave 211 P5 (final §Methods experiment-setup doc for TPAMI submission
flow document `docs/drafts/paper-flattened-draft.md`).
**Inputs:**
...
- `verification_outputs/ckpt_sha256.json` (checkpoint SHA-256 ledger)
- `docs/drafts/paper-flattened-draft.md` §3.3 Table 3.2 (per-cell R-level config)
- `docs/INSIGHTS.md` (12-col audit format reference)
- `docs/lean/FLOWA_INTEGRATION.md` (12-adapter inventory)

This document supersedes the Wave 209 P8 logical-redo
`docs/audit/wave209-p8-experiment-setup.md` as the **paper-supplement**
final §Methods. Wave 209 P8 remains in-tree as a historical reference.
...
### 2.4 SHA-256-pinned checkpoints

Per `verification_outputs/ckpt_sha256.json`:
...
Reviewers verify with `sha256sum <path>` (see `RELEASE-NOTES-v3.0.md`
§Reproducibility for the canonical command).
...
| Cell | Domain | Model | Baseline NFE | Framework NFE | n_rounds | Per-round NFE | n_seeds | n_paired | Output path |
|---|---|---|---|---|---|---|---|---|---|
| **R1** | protein FM | LineageFlow (ICML 2026, 657M params, ESM-2 33-token) | 50 | 150 | 3 | 50 | 6 | 1000 | `verification_outputs/wave195-p2-r-level-power.csv` |
| **R2** | protein flow-AE | Kanzi (ICLR 2026, 44.1M params) | 50 | 150 | 3 | 50 | 3 | 1000 | `verification_outputs/wave206-p2-kanzi-framework-n1000-audit.csv` |
| **R3** | molecular 3D FM | FlowMol3 (NeurIPS 2024, 65M params, DGL graph kernels) | 250 | 250 | 3 | 83.33 | 1 (DGL 2.4.0 wheel gap) | 1000 | `verification_outputs/flowmol3_n1000_*.csv` |
| **R5a** | 2D synthetic FM | Two Moons (analytic target) | 100 | 100 | 3 | 33.33 | 7 (Wave 209 P6 E2) | 3 per seed (unpaired) | `docs/r4-survey/two_moons_*.csv` |
| **R5b** | image RF | Rectified Flow CIFAR-10 (DDPM++ UNet, 32×32) | 50 | 50 | 4 | 12.5 | 1 | 1000 (10 chunks × 100) | `verification_outputs/wave209-p4-wallclock.csv` |
| **R5c** | image FM | MNIST FM (Open recipe) | 50 | 25 | 3 | 8.33 | 1 | 1000 (10 chunks × 100) | `verification_outputs/wave209-p4-wallclock.csv` |
| **R6** | protein FM | LineageFlow (k6 foldability) | 50 / 200 | 50 / 200 | 3 | 16.67 / 66.67 | 6 | 1000 (4 Pfam × 250) | `verification_outputs/wave206-p1-lineageflow-n1000-audit.csv` |
...
| Cell | n_seeds | n_paired per seed | n_paired total | audit-grade unit | seed list |
|---|---|---|---|---|---|
| R1 LineageFlow HMMER | 6 | n/a (per-record paired) | 1000 | per-record | `[42, 43, 44, 45, 46, 47]` |
| R2 Kanzi inv-proj | 3 | n/a (per-record paired) | 1000 | per-record | `[42, 43, 44]` |
| R3 FlowMol3 fg_dev | **1** | 999 / 1000 | 1000 | per-record (DGL 2.4.0 wheel gap blocks 3-seed re-run) | `[42]` |
| R5a 2D Two Moons | **7** (Wave 209 P6 E2) | 3 (per-seed unpaired) | 3 per seed | per-seed | `[0, 1, 2, 3, 4, 5, 6]` |
| R5b CIFAR-10 RF NFE=50 | 1 | 10 chunks × 100 | 1000 | per-chunk (df=9) | `[42]` |
| R5c MNIST FM NFE=50 | 1 | 10 chunks × 100 | 1000 | per-chunk (df=9) | `[42]` |
| R6 k6 foldability | 6 | n/a (per-record paired) | 1000 (4 Pfam × 250) | per-record | `[42, 43, 44, 45, 46, 47]` |

### 4.2 Per-seed vs per-record breakdown

**Per-seed cells** (R5a 2D): the unit of audit-grade evidence is the
**seed**; `n_paired = 3` per seed gives df=2 per-seed t-statistics,
and the headline statistic is the **per-seed mean paired delta ± 1 SE**
across the 7 seeds (one-sample t vs zero, df=6).

**Per-record cells** (R1, R2, R3, R5b, R5c, R6): the unit of audit-grade
evidence is the **record**; `n_paired = 1000` records per cell gives
df=999 per-record paired t-statistics. The cluster-robust re-analysis
on R6 reduces this to df_cluster=3 at the Pfam-family unit.
...
### 6.7 D.4 byte-stable regression gate
...
- **Hardware documented:** i9-14900K (24C/32T) + 2× sm_120 Blackwell
  GPUs (RTX PRO 6000 98 GB primary, RTX 5090 32 GB secondary);
...
- **SHA-256-pinned checkpoints:** 4 ckpts, `verification_outputs/ckpt_sha256.json`.
- **D.4 byte-stable regression:** 18 adapters × 33 vectors, all PASS.
- **Claims ledger:** 65 total entries (62 ACTIVE + 1 PROVISIONAL + 2
  DEPRECATED), `tools/check_claims_consistency.py` enforces.

---

## 9. Supersession note

This document **supersedes** `docs/audit/wave209-p8-experiment-setup.md`
(Wave 209 P8 logical redo) as the **paper-supplement final** §Methods.
Wave 209 P8 remains in-tree as a historical reference; both versions
carry identical headline numbers (the §3.1 R-level cell table, the §4
seed counts, the §5 wall-clock budgets, and the §6 reproducibility
commands are unchanged across versions). The only substantive
additions in Wave 211 P5 are: (i) explicit CPU documentation (i9-14900K),
(ii) per-cell `output path` column in §3.1, (iii) explicit per-seed
seed lists in §4.1, and (iv) the §6.7 D.4 gate + §6.8 claims ledger
