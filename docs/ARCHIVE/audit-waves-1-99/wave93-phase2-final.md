...
**Wave 99 ADDITIVE update — real N=1000 Kanzi verdict + statistical power analysis (does NOT delete the Wave 96.D / Wave 96.E rows above).** Wave 99.A produced a docs-only refresh of `docs/baseline-audit-report.md` (commit `1f6bab5`); Wave 99.A did NOT run a new framework-arm sweep. Wave 99.B therefore re-states the verdict using the Wave 96.E N=10 framework arm + Wave 88 N=1000 baseline arm (the most recent real framework paper-metric data available — `verification_outputs/kanzi_n1000_real_v2/per_metric.jsonl` does NOT exist on the working tree, so the "1000 records" path the task brief expected is unavailable). Wave 99.B runs `tools/statistical_power_analysis.py` per-metric + Bonferroni-corrected analysis on all 6 Kanzi paper metrics with the task-brief CLI values (`--framework-mean 1.766 --framework-std 0.214 --baseline-mean 0.902 --baseline-std 0.137 --baseline-n 1000 --framework-n 1000`). **The headline finding is that W2 ("framework paper-metric unverifiable at N=1000") is NOT closed by Wave 99 — the framework arm at N=1000 has not been executed on real Kanzi ckpt + paper metrics.** The Wave 99.B row reads:

> | model | metric | N (framework) | N (baseline) | baseline | framework | Δ (Å) | 95% CI (Å) | p (raw) | p (Bonf) | power@1pp | verdict |
> |---|---|---:|---:|---:|---:|---:|---|---:|---:|---:|:---|
> | kanzi | `reconstruction_kabsch_rmsd_A` (Wave 99.B) | **10** (framework) / 1000 (baseline) | **0.902 ± 0.137** (Wave 88 N=1000) | **1.766 ± 0.214** (Wave 96.E N=10 production) | **+0.864** | **[+0.671, +1.056]** | **4.6e-7** | **4.6e-7** | **0.051** | **`UNDERPOWERED`** per Wave 93 verdict precedence — Bonferroni-significant REGRESSES on the metric (p = 4.6e-7 ≪ 0.0083), but post-hoc power @ 1pp = 0.05 < 0.5 ⇒ UNDERPOWERED takes precedence; honest-vacuum criterion: informative for direction (Δ > 0, framework worse) but not for magnitude (CI ±0.19 Å); W2 NOT closed by Wave 99.A (docs-only); W2 NOT closed by Wave 99.B (no new sweep); closing W2 requires Wave 100 N=1000 framework sweep (~16.7 h CPU on `kanzi_venv`) |

The verdict precedence is `TIE → UNDERPOWERED → SUPPORTED → REGRESSES → NOT_SIGNIFICANT`. The same data with the classical Welch-test-only framing (no power-as-precedence) reads `REGRESSES_BY_+0.86_Å` on `reconstruction_kabsch_rmsd_A` at Bonferroni p = 4.6e-7 — the effect exists with high confidence; the magnitude is bounded by the 95% CI to [+0.671, +1.056] Å at N=10. The 5 codebook metrics are TIED_BY_DESIGN (Wave 92c §3: framework restart-blend acts on flow trajectory, not on the post-reconstruction FSQ round-trip); per-arm scalars are diagnostic only. See `docs/audit/wave99b-n1000-verdict.md` for the full Wave 99.B statistical power analysis (per-metric + Bonferroni + Wave 93 verdict precedence + 6-cell table + W2 status + forward Wave 100 plan).
