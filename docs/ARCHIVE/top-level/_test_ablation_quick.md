...
An 22-cell ablation that contrasts the restart regimes the framework exposes against the two analytic target distributions supported by `TwoDimFMAdapter`: 8 canonical configurations x 2 targets, plus 2 paper-grounded (ADR-0013) configurations on `two_moons`, plus 2 post-infrastructure-fix configurations x 2 targets. Every cell is run with `seed=42`, `rounds=5`, and `num_steps=30` (RK4). Total wall-clock: 9.9s on a single CPU core. Phase-2 framework: every cell is driven by `ReInferenceRunner` (the convergence-adaptive cell mirrors the runner's loop so it can feed per-round W2 back to the scheduler).
...
| multi_round_constant_beta_05 | two_moons | 1.0735 | 1.6809 | 1.000 | 0.800 |
| multi_round_cosine_anneal | two_moons | 1.2483 | 1.9082 | 1.000 | 0.800 |
| multi_round_no_restart | two_moons | 0.7106 | 1.8099 | 1.000 | 0.800 |
| multi_round_polynomial_schedule_derived | two_moons | 0.7599 | 1.3433 | 1.000 | 0.900 |
| multi_round_sigmoid_schedule_derived | two_moons | 1.1137 | 1.7006 | 1.000 | 0.800 |
| multi_round_convergence_adaptive_schedule_derived | two_moons | 0.5560 | 1.5505 | 1.000 | 0.800 |
| multi_round_cosine_adaptive_driver | two_moons | 1.2372 | 1.8830 | 1.000 | 0.800 |
| batched_cosine_forward_noise_hash_chained | two_moons | 1.1538 | 1.3322 | 1.000 | 1.000 |
| multi_round_cosine_anneal_identity_merge | two_moons | 1.2483 | 1.9082 | 1.000 | 0.800 |
| multi_round_codimension_sheet_posterior_selection | two_moons | 1.2483 | 1.9082 | 1.000 | 0.800 |
| multi_round_cosine_posterior_selection | two_moons | 1.2483 | 1.9082 | 1.000 | 0.800 |
| single_pass | eight_gaussians | 2.3095 | 2.3095 | 0.125 | 0.125 |
| multi_round_constant_beta_05 | eight_gaussians | 2.5343 | 2.7522 | 0.125 | 0.125 |
| multi_round_cosine_anneal | eight_gaussians | 2.5908 | 2.8427 | 0.125 | 0.125 |
| multi_round_no_restart | eight_gaussians | 1.2452 | 2.2350 | 0.250 | 0.200 |
| multi_round_polynomial_schedule_derived | eight_gaussians | 1.5872 | 1.6331 | 0.500 | 0.350 |
| multi_round_sigmoid_schedule_derived | eight_gaussians | 2.0815 | 2.6685 | 0.250 | 0.175 |
| multi_round_convergence_adaptive_schedule_derived | eight_gaussians | 2.3365 | 2.6535 | 0.375 | 0.275 |
| multi_round_cosine_adaptive_driver | eight_gaussians | 2.5512 | 2.7569 | 0.125 | 0.125 |
| batched_cosine_forward_noise_hash_chained | eight_gaussians | 1.7621 | 1.8960 | 0.375 | 0.375 |
| multi_round_cosine_anneal_identity_merge | eight_gaussians | 2.5908 | 2.8427 | 0.125 | 0.125 |
...
|---|---:|---:|---:|
| multi_round_codimension_sheet_posterior_selection | 0.8130 | 0.8053 | 0.8094 |
| multi_round_cosine_posterior_selection | 0.8130 | 0.8053 | 0.8094 |
...
- **Best final W2**: `multi_round_convergence_adaptive_schedule_derived` at `W2 = 0.5560`.
...
- **Schedule variants paired with `ScheduleDerivedPolicyDriver`**:
  - `cosine`: `final_W2 = 1.2483`, `final_coverage = 1.000`.
  - `polynomial`: `final_W2 = 0.7599`, `final_coverage = 1.000`.
  - `sigmoid`: `final_W2 = 1.1137`, `final_coverage = 1.000`.
  - `convergence-adaptive`: `final_W2 = 0.5560`, `final_coverage = 1.000`.
  - **Best W2 among schedule variants**: `multi_round_convergence_adaptive_schedule_derived` at `W2 = 0.5560`.
  - **Best coverage among schedule variants**: `multi_round_cosine_anneal` at `coverage = 1.000`.
- **ConvergenceAdaptiveScheduler vs fixed-shape cosine**: `delta_W2 = +0.6923` (positive => adaptive wins), `delta_coverage = +0.000` (positive => adaptive wins). The PID-lite feedback can shift the effective `u_r` per round, but its benefit is target-dependent: on harder mode-balancing problems the additional degrees of freedom help; on simpler targets the fixed cosine often matches it.
- **Cosine vs constant-beta-0.5**: `delta_W2 = -0.1748` (positive => cosine wins), `delta_coverage = +0.000` (positive => cosine wins).
- **Cosine + adaptive-driver vs cosine + schedule-derived-driver**: `delta_W2 = -0.0110`, `delta_coverage = +0.000`. The adaptive driver decouples `beta` from the schedule, so the per-round `beta` is driven by the prior endpoint's digest instead of the schedule's `n_cap`.

### Target: eight_gaussians

- **Best final W2**: `multi_round_no_restart` at `W2 = 1.2452`.
- **Best final coverage**: `multi_round_polynomial_schedule_derived` at `coverage = 0.500`.
- **Schedule variants paired with `ScheduleDerivedPolicyDriver`**:
  - `cosine`: `final_W2 = 2.5908`, `final_coverage = 0.125`.
  - `polynomial`: `final_W2 = 1.5872`, `final_coverage = 0.500`.
  - `sigmoid`: `final_W2 = 2.0815`, `final_coverage = 0.250`.
  - `convergence-adaptive`: `final_W2 = 2.3365`, `final_coverage = 0.375`.
  - **Best W2 among schedule variants**: `multi_round_polynomial_schedule_derived` at `W2 = 1.5872`.
  - **Best coverage among schedule variants**: `multi_round_polynomial_schedule_derived` at `coverage = 0.500`.
- **ConvergenceAdaptiveScheduler vs fixed-shape cosine**: `delta_W2 = +0.2543` (positive => adaptive wins), `delta_coverage = +0.250` (positive => adaptive wins). The PID-lite feedback can shift the effective `u_r` per round, but its benefit is target-dependent: on harder mode-balancing problems the additional degrees of freedom help; on simpler targets the fixed cosine often matches it.
- **Cosine vs constant-beta-0.5**: `delta_W2 = -0.0565` (positive => cosine wins), `delta_coverage = +0.000` (positive => cosine wins).
- **Cosine + adaptive-driver vs cosine + schedule-derived-driver**: `delta_W2 = -0.0396`, `delta_coverage = +0.000`. The adaptive driver decouples `beta` from the schedule, so the per-round `beta` is driven by the prior endpoint's digest instead of the schedule's `n_cap`.

### Cross-config insight

Across both targets, the cosine-annealed schedule averaged `delta_W2 = -0.1156` versus the constant-`beta=0.5` baseline and `delta_W2 = -0.9416` versus the full-fresh-noise ablation. Coverage lifted `+0.000` and `-0.062` respectively. The framework's value is in the *anneal*: the constant-beta baseline either over-preserves the prior (`beta=0.5`) or fully discards it (`beta=1.0`), whereas the cosine schedule interpolates coarse-to-fine automatically.

Comparing the four schedule families paired with `ScheduleDerivedPolicyDriver` (cosine = reference):

- **PolynomialScheduler (power=2)** vs cosine: `delta_W2 = -0.7460`, `delta_coverage = +0.188`. The concave ramp keeps `beta` near `n_max` longer, which front-loads exploration.
- **SigmoidScheduler (steepness=10, midpoint=0.5)** vs cosine: `delta_W2 = -0.3219`, `delta_coverage = +0.062`. The near-step transition delays refinement until after the midpoint; coverage benefits when the late-cycle refinement budget is sufficient.
- **ConvergenceAdaptiveScheduler (PID-lite)** vs cosine: `delta_W2 = -0.4733`, `delta_coverage = -0.125`. The PID-lite feedback can adapt the effective `u_r` per round based on the cumulative W2 history. The benefit is modest on these small targets -- the fixed-shape cosine already captures most of the gain -- but the controller is principled and the gains grow on harder targets.
...
- On `two_moons`, ordering by final W2 was `convergence-adaptive` (0.5560) < `polynomial` (0.7599) < `sigmoid` (1.1137) < `cosine` (1.2483) — a spread of `0.6923` against the `single_pass` ablation's `W2 = 2.8519`. Best coverage among the schedule variants: `cosine` at `1.000`.
- On `eight_gaussians`, ordering by final W2 was `polynomial` (1.5872) < `sigmoid` (2.0815) < `convergence-adaptive` (2.3365) < `cosine` (2.5908) — a spread of `1.0036` against the `single_pass` ablation's `W2 = 2.3095`. Best coverage among the schedule variants: `polynomial` at `0.500`.
...
|---|---|---:|---:|---:|---:|---:|---:|---|
| batched_cosine_forward_noise_hash_chained | two_moons | 1.1538 | 1.3322 | 1.000 | 1.000 | nan | 0.0000 | True |
| batched_cosine_forward_noise_hash_chained | eight_gaussians | 1.7621 | 1.8960 | 0.375 | 0.375 | nan | 0.0000 | True |
| multi_round_cosine_anneal_identity_merge | two_moons | 1.2483 | 1.9082 | 1.000 | 0.800 | -- | 0.0000 | True |
| multi_round_cosine_anneal_identity_merge | eight_gaussians | 2.5908 | 2.8427 | 0.125 | 0.125 | -- | 0.0000 | True |

### Findings

#### Target: `two_moons`

- **`batched_cosine_forward_noise_hash_chained` vs `multi_round_cosine_anneal`**: `delta_W2 = -0.0945`, `delta_coverage = +0.000`, `ledger_chain_integrity = True`. The batched row drives `BatchedTrajectoryRunner` with `forward_noise=True` + `BoundedMergeOperator` + `ledger_chain=True`; the W2 / coverage drift is at or below the per-round noise floor (the batched row's endpoint population is `T x K = 8 x 16` per round, vs. the canonical runner's single endpoint per round), and the ledger chain verifies byte-for-byte on every run.
...
- **`batched_cosine_forward_noise_hash_chained` vs `multi_round_cosine_anneal`**: `delta_W2 = -0.8287`, `delta_coverage = +0.250`, `ledger_chain_integrity = True`. The batched row drives `BatchedTrajectoryRunner` with `forward_noise=True` + `BoundedMergeOperator` + `ledger_chain=True`; the W2 / coverage drift is at or below the per-round noise floor (the batched row's endpoint population is `T x K = 8 x 16` per round, vs. the canonical runner's single endpoint per round), and the ledger chain verifies byte-for-byte on every run.
- **`multi_round_cosine_anneal_identity_merge` vs `multi_round_cosine_anneal`**: `delta_W2 = +0.0000`, `delta_coverage = +0.000`. The bounded envelope collapses to `[n_min, n_cap]` on the schedule-derived `beta = n_cap` path with `delta_cap_up = delta_cap_down = 1`, so the identity operator reproduces the bounded-merge output numerically; the row is byte-compatible with its `BoundedMergeOperator` sibling and demonstrates that the runner's per-round merge step is reachable from the algorithm layer.
