...
- `verification_outputs/wave206-p2-kanzi-framework-n1000.{csv,json}` — R2 Kanzi `framework_inv_proj` byte-stable reference — **SUPERSEDED by Wave 214 P2 fix** (mean_diff = +0.656 Å, d_z = +3.532 was derived from the Wave 178+196 source regression that bypassed the Wave 95.P3.B trained-inverse bridge; Wave 214 P1 audit identified root cause, Wave 214 P2 restored bridge in `tools/_kanzi_sweep_runner.py:_synthesize_x_final_real`, Wave 214 P2 smoke test (N=10) confirmed framework mean returns to 0.8758 Å ≈ Wave 127 byte-stable 0.8798 Å) — see `docs/audit/wave214-p3-clm057-update.md` for the user-directed verdict correction
