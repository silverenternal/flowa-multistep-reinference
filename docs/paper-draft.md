...
Flow matching [Lipman 2023] and Rectified Flow [Liu 2022] define generation as integrating a learned velocity field $v_\theta(x, t)$ along a single ODE. We show that for frozen checkpoints, **two distinct improvements are possible by re-querying the same model with restart-blending**: at matched NFE, framework produces measurably better samples; at matched sample quality, framework uses 2.5–10× fewer NFE. We present **FlowA**, an implementation of this re-inference principle with structural guarantees from JMAA Theorem 1 (Li 2026, the author's BL-convergence rate bound for the noised sheet-measure framework). On three 2026 SOTA real checkpoints — Kanzi (ICLR 2026 protein flow-AE), LineageFlow (ICML 2026 protein FM), FlowMol3 (NeurIPS 2024 molecular 3D FM) — FlowA achieves composite lift +0.1695 byte-stable across NFE 10–2000 (Kanzi) and +0.2083 byte-stable across NFE 10–200 (LineageFlow) at matched NFE; and reaches 2D FM baseline saturation in 10× fewer NFE and CIFAR-10 Rectified Flow baseline quality in 2.5× fewer NFE at matched quality. Re-inference is orthogonal to training — an inference-time control problem.
...
**Contributions.** We organize around the two central claims above. (i) *Matched-quality framework improvement* — composite lift **+0.1695** on Kanzi byte-stable across NFE 10–2000 (§7.3), **+0.2083** on LineageFlow byte-stable across NFE 10–200 (§7.4), 2D Two Moons W₂ −7.28% at matched NFE 500 (§4.2), CIFAR-10 RF FID −44.17% at NFE=2 (§4.3). (ii) *Matched-NFE speedup* — 2D FM in 10× fewer NFE (§4.2), CIFAR-10 RF in 2.5× fewer NFE (§4.3). Both supported by **17 typed state machines / 333 typed transitions** wired by four pluggable feedback loops, with structural guarantees from **JMAA Theorem 1 BL-convergence rate bound** (§3.2). Framework is **training-free** (no retraining / distillation / Reflow), **solver-agnostic** (stacks on Euler, Heun, DPM-Solver++), and **paper-quantity-driven re-inference** (a new axis not occupied by DPM-Solver++ or Consistency Models). Two honest negatives: matched-NFE CIFAR-10 FID is 24–31% worse than the constant-NFE baseline at moderate NFE (§4.3); FlowMol3 verdict REMAINS TIE_AT_SATURATION on chemistry axis at GAP-4 (§7.5). Two honest reframings: Tier 3 metrics saturate at NFE=10 by metric property (validity_rate ceiling), so the framework's value-add is **NFE-independent composite lift, not NFE-acceleration** (§7.7.7) — it reaches a different endpoint, not the same endpoint sooner.
