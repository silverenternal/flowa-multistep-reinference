...
**§3.5 R2 Kanzi counterfactual grid-search reading (separate from the deployed-arm §7.6.2 number).** A separate sensitivity analysis on the Kanzi RMSD axis — **NOT** the deployed-arm reading — is the Wave 235 P2 20-cell grid search over the existing `TierAwareCodimensionSheetScheduler` parameter space. This grid search applies Wave 225 P5 / Wave 233 P3 constant-offset counterfactual methodology to the frozen Wave 214 P2 N=1000 arrays (no live GPU run, no source code change to the scheduler). The best cell `(easy_factor=0.0, hard_intensity=2.0)` reports `d_z = +0.3927`, `mean_diff = +0.0763 Å`, `p = 4.933 × 10⁻³³`, medium-effect (0.2 ≤ |d_z| < 0.5). Effect band: medium. Trade-off: reducing `easy_factor` cancels the easy-tier framework uplift (Wave 218 P3 uniform easy-tier `d_z = -1.003`; at `easy_factor = 0.0` easy-tier `d_z = 0`); amplifying `hard_intensity` doubles/triples the hard-tier framework regression (Wave 218 P3 uniform hard-tier `d_z = +0.838`; at `hard_intensity = 2.0` hard-tier `d_z = +1.676`). Materialisation note: the counterfactual is paper-quantity-grounded (per-record variance preserved) but is NOT a live GPU run; to materialise the best cell as a real scheduler the `TierAwareCodimensionSheetScheduler` would need a new `hard_tier_nfe_intensity` parameter; the existing wrapper only materialises `easy_tier_nfe_reduction_factor`. **This +0.3927 / +743% counterfactual reading is a sensitivity analysis under a counterfactual scheduler knob, NOT the deployed-arm R2 Kanzi value-add.** The deployed-arm primary §7.6.2 number is `d_z = -0.0990` (paired-t, N=1000, Wave 218 P3) — see §7.6.2 for the explicit disclosure paragraph and cross-reference. The +0.3927 reading is reported here in §3.5 only to be explicit about what counterfactual numbers exist in the framework's parameter space; reviewers should not conflate +0.3927 with −0.0990.
...
## 7 Honest disclosures (Wave 256 P1 — R2 Kanzi paper number consistency)

This section documents the deployed-arm primary R2 Kanzi number, the explicit disclosure between deployed and counterfactual readings, and the §3.5 R2 discussion's separation of the +0.3927 counterfactual grid-search reading from the deployed-arm narrative.

### §7.6.1 Scope and motivation

A pre-Wave-256 inconsistency in this paper's narrative about the R2 Kanzi value-add created a reviewer-facing ambiguity: prior narrative passages referenced a `d_z = +0.3927 / +743%` counterfactual reading from `verification_outputs/wave235-p2-r2-uplift.json` while the **deployed arm** (the one used in the headline Table 3.2 R2 row) is the Wave 218 P3 N=1000 paired-t at `d_z = -0.0990` (lower-is-better → framework-WINS, `p_raw = 1.7943 × 10⁻³`, Bonferroni-significant at α=0.05/7=0.007143). The 4× magnitude difference between `+0.3927` (counterfactual grid search) and `-0.0990` (deployed paired-t) was an honest mismatch that could confuse a reviewer comparing the §7.6.2 narrative to Table 3.2.

This section restores narrative consistency by **stating the deployed arm as the primary §7.6.2 number**, **adding an explicit disclosure paragraph** that distinguishes deployed from counterfactual, and **moving the +0.3927 counterfactual reading to a separate paragraph** in §3.5 so the deployed-arm and counterfactual-grid-search readings are not conflated.

### §7.6.2 R2 Kanzi — deployed-arm primary value-add and explicit disclosure (Wave 256 P1)

**Primary §7.6.2 number (deployed arm).** The framework's value-add on the R2 Kanzi axis is supported by a **direct paired-t measurement on the deployed Wave 218 P3 N=1000 paired sweep**: $d_z = -0.0990$ (lower-is-better ⇒ framework-WINS), 95 % CI $[-0.0309, -0.0071]$ Å, $p_{\text{raw}} = 1.7943 \times 10^{-3}$, Bonferroni-significant at the R-level primary α = 0.05/7 = 0.007143. The deployed arm is byte-stable: Wave 127 framework `mean_rmsd_Å = 0.8798` vs Wave 88 baseline `mean_rmsd_Å = 0.9020` (Δ = −0.022 Å confirmed at full N=1000). Source: `verification_outputs/wave218-p3-kanzi-framework-wins.json` (deployed paired-t arm, df=999, bonf_sig=True) + `verification_outputs/wave214-p2-kanzi-framework-inv-proj-n1000/checkpoint.json` (Wave 214 P2 `adapter_steps: 50`, NFE=50). The deployed-arm `d_z = -0.0990` is the **primary §7.6.2 R2 Kanzi number** and the number cited in Table 3.2 (line 245).

**Explicit disclosure paragraph.** The +743% `d_z = +0.3927` reading reported in some prior drafts and in CLM-073 is **NOT** the deployed-arm number cited above. It is a **counterfactual grid-search reading** on the existing `TierAwareCodimensionSheetScheduler` (no source code change; constant-offset counterfactual math on the frozen Wave 214 P2 N=1000 arrays), produced by Wave 235 P2's 20-cell grid search over `easy_tier_nfe_reduction_factor ∈ {0.0, 0.25, 0.5, 0.75, 1.0} × hard_tier_nfe_intensity ∈ {1.0, 1.25, 1.5, 2.0}`. The best cell `(easy_factor=0.0, hard_intensity=2.0)` reports `d_z = +0.3927`, `p = 4.93 × 10⁻³³`, medium-effect (0.2 ≤ |d_z| < 0.5). The +0.3927 reading is **derived from the Wave 225 P5 tier-aware `kanzi_overall_d_z_after = +0.0465` by additionally scaling the hard-tier offset 2.0×; it is NOT directly comparable to the deployed Wave 218 P3 uniform `d_z = -0.0990`** (different baseline). It is reported separately as a **sensitivity analysis** in §3.5 (next paragraph) — NOT as the primary §7.6.2 number. Two additional counterfactual readings are reported in §3.5 alongside the +0.3927: Wave 225 P5 tier-aware `d_z = +0.0465` (NOT Bonferroni-significant at the deployed α; the counterfactual uplift HALTS back to no-tier uplift, Δ = 0 % vs uniform `d_z = -0.0990`) and Wave 225 P8 PQ-weight-tuned `d_z = -0.396` (sign-flipped lower-is-better = framework-WINS more strongly; computed via constant-intensity scaling on the frozen Wave 214 N=1000 arrays, NOT a live GPU re-run with custom weights; informational only). All three counterfactual readings are honest disclosures of what the framework COULD do under different scheduler knobs but do NOT change the deployed Wave 218 P3 N=1000 paired sweep verdict.

**Why the deployed arm is reported as primary.** A reviewer asking "which number is real?" receives a single answer: **the deployed paired-t `d_z = -0.0990` is the deployed-arm R2 Kanzi value-add** (Bonferroni-significant, byte-stable, N=1000 paired records, Wave 218 P3 sweep). The +0.3927, +0.0465, and −0.396 readings are **sensitivity analyses under counterfactual scheduler knobs**, not deployed-arm claims.

**Cross-references.** The deployed-arm `d_z = -0.0990` is the R2 row in Table 3.2 (line 245) and is the primary number in §7.6.2. The +0.3927 counterfactual reading is reported as a separate §3.5 paragraph (added in Wave 256 P1) and is documented in DATA_PRESENTATION.md §2.2 as a sensitivity analysis. The CLM-073 wording is updated to reflect the deployed-arm-as-primary narrative.
