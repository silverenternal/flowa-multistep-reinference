...
FlowA closes this gap through a single theorem-driven interface. We state Theorem 1 within this paper: under F-side hypotheses on a compact codimension-1 sheet fibre and uniformly separated root cells of the framework's posterior, the bounded-Lipschitz distance between the framework's sampling measure and the ODE target measure is bounded above by $A_g \cdot \exp(-\mathrm{NFE}/B_g) + C_g \cdot e_\rho$, where $A_g$ is the Lipschitz aggregate of the velocity estimator, $B_g$ is the effective NFE decay rate, $C_g$ is the per-cell residual bias from sheet-versus-cell evidence imbalance, and $e_\rho$ is the exponential exterior-gap constant $\min\{\rho^4, (1-\rho)^2 \eta^2\}$ quantifying the rate at which the physical complement is suppressed. We derive the four quantities as closed-form expressions of the adapter's posterior geometry at runtime and pass them directly as scheduler inputs: the CodimensionSheetScheduler consumes $(A_g, B_g, C_g)$ to allocate the per-round noise-and-step budget, the BoundedMergeOperator consumes $e_\rho$ to enforce a non-zero noise floor that keeps the posterior on the fibre, and the EvidenceDrivenScheduler consumes the same quadruple to drive the policy's per-cell restart probability. The framework operates without retraining, distillation, or Reflow; stacks on Euler, Heun, DPM-Solver++, Dormand–Prince RK45, CTMC, and BFN solvers; and exposes the inference loop as a typed four-port control surface — `SchedulerProtocol`, `PolicyDriverProtocol`, `MergeOperatorProtocol`, `RestartBlenderProtocol` — that a domain expert can drive without manipulating the flow matching internals.
...
|---|---:|---:|---:|---:|---:|---:|---|---:|---|---|---:|:---:|
| **R1** LineageFlow hmmscan | 1000 (unpaired) | +0.1840 | — | 5.697 | 1998 | 1.49 × 10⁻⁸ | [+0.121, +0.247] | +0.255 (d_s) | Welch t | R-level primary | 0.007143 | **YES** |
| **R2** Kanzi inv-proj rmsd_Å | 1000 | +0.0184 | 0.1925 | 3.023 | 999 | 2.57 × 10⁻³ | [+0.0065, +0.0303] | +0.096 (d_z) | paired t | R-level primary | 0.007143 | **YES (raw); post-hoc-power UNDERPOWERED** |
| **R3** FlowMol3 fg_dev | 1000 (unpaired) | −0.0235 | — | −2.877 | 1997 | 4.00 × 10⁻³ | [−0.0395, −0.0075] | −0.129 (d_s) | Welch t | R-level primary | 0.007143 | NO (post-hoc-power UNDERPOWERED) |
...
| **R5b** CIFAR-10 RF NFE=50 FID | 1000 (paired) | +90.045 | — | 8.539 | 999 | 1.31 × 10⁻⁵ | [+69.378, +110.712] | +2.700 (d_z) | paired t | R-level primary | 0.007143 | NO (REGRESSES by direction; **boundary characterization**) |
| **R5c** MNIST FM NFE=50 FID | 1000 (paired) | −6.105 | 1.465 | −41.66 | 999 | 1.32 × 10⁻¹¹ | [−6.392, −5.817] | −13.175 (d_z) | paired t | R-level primary | 0.007143 | **YES** |
| **R6** k6 overall pLDDT | 1000 | +1.123 | 15.880 | 2.237 | 999 | 2.55 × 10⁻² | [+0.139, +2.107] | +0.071 (d_z) | paired t | R-level primary | 0.007143 | NO (cluster-robust $p_{\text{cluster}} = 5.53 \times 10^{-1}$ → UNDERPOWERED) |
| **R6** k6 overall scPerplexity | 1000 | −3.917 | 3.638 | −34.047 | 999 | 2.74 × 10⁻¹⁶⁹ | [−4.142, −3.691] | −1.077 (d_z) | paired t | R-level primary | 0.007143 | **YES** (cluster-robust $p_{\text{cluster}} = 4.02 \times 10^{-3}$) |
