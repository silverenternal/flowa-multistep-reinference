...
def build_final_restart_policy(
    *,
    ...
    schedule_sample: Optional[CosineScheduleSample],
    ...
    * ``policy_hash`` is recomputed deterministically via
    ...
    beta_keys = set(str(k) for k in beta_by_channel.keys())
    alpha_keys = set(str(k) for k in alpha_by_channel.keys())
    floor_keys = set(str(k) for k in fresh_noise_floor_by_channel.keys())
    freeze_keys = set(str(k) for k in freeze_admission_by_channel.keys())
...
def verify_policy_against_ledger(
    ...
    * ``ledger.finite_prefix_only`` is ``True``.
    ...
    policy_beta_keys = sorted(str(k) for k in policy.beta_by_channel.keys())
    ledger_beta_keys = sorted(str(k) for k in ledger.beta_by_channel.keys())
    ...
    if bool(ledger.finite_prefix_only) is not True:
        return False
