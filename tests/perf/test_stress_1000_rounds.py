"""Long-horizon 1000-round x 8-capability stress test.

Drives :class:`adaptive_reflow.frame.engine.Engine` for 1000
consecutive ``run_round`` invocations against the universal
:class:`ToyGaussianAdapter` and asserts:

* All 1000 round traces emit empty ``audit_codes`` (every fail-closed
  gate passes for 1000 rounds in a row).
* All 1000 ledger row IDs are unique (the engine's per-round hash
  payload includes ``round_index``, so uniqueness follows from the
  monotonic advance).
...
* Phase state advances deterministically: ``round_in_cycle`` is a
  strict counter and ``recorded_at_round`` always equals the round
  index.
* The ledger's pickled byte size grows linearly in the round count
  (per-round delta is bounded — no super-linear spikes).
* Two 1000-round runs produce byte-identical final-state hashes.

The 8-capability mention in the module docstring refers to the
``Engine.handshake`` surface that the adapter must satisfy
(``has_ode_integration_surface`` / ``has_prior_export`` /
``has_state_export`` / ``has_condition_injection`` /
``has_restart_boundary`` / ``has_trajectory_digest`` /
``has_deterministic_seed`` / ``has_materialization_route``). Every
round exercises all eight.

All tests are decorated with ``@pytest.mark.stress`` so they run on
the stress-nightly workflow only.
"""
...
# Preflight: this stress test is decorated with ``@given`` /
# ``@settings`` so it requires the ``hypothesis`` package. On
# sandboxes where hypothesis is not vendored, pytest collection
# aborts with ``ModuleNotFoundError``. The importorskip below
# short-circuits collection cleanly: every test in this module
# reports SKIP rather than failing or aborting the suite.
pytest.importorskip(
    "hypothesis",
...
# ToyGaussianAdapter advertises a single ``"x"`` channel; the universal
# engine handshake requires all 8 capabilities, hence "8-channel".
...
def _make_phase_state(*, horizon_remaining: int = 2 * NUM_ROUNDS) -> PhaseState:
    ...
    return PhaseState(
        ...
        seed_lineage_digest="stress:seed-lineage-1000",
        recorded_at_round=0,
    )
...
def _make_final_policy(
    *,
    policy_id: str = "policy-stress-1000",
    run_id: str = "run-stress-1000",
    target_round: int = 0,
) -> FinalRestartPolicy:
    ...
    policy = FinalRestartPolicy(
        ...
        outer_cycle_id=0,
        beta_by_channel={
            ChannelName(k): FactorValue(0.0) for k in GAUSSIAN_CHANNELS
        },
        alpha_by_channel={
            ChannelName(k): FactorValue(1.0) for k in GAUSSIAN_CHANNELS
        },
        ...
        freeze_admission_by_channel={
            ChannelName(k): True for k in GAUSSIAN_CHANNELS
        },
...
def _make_condition_delta(
    *,
    ...
    source: str = "stress-1000",
    calibration_artifact_hash: str = "cal-stress-1000",
) -> ODEConditionDelta:
...
def _build_source_bundle(adapter, *, round_index: int) -> StateBundle:
    """Return a detached, valid source :class:`StateBundle`.

    Each round rebuilds the bundle via :meth:`FlowMatchingODEAdapter.build_initial_state`
    so the bundle is structurally valid (capability handshake OK,
    ``source_round`` is a non-negative int, ``detach_proof=True``,
    the ``"x"`` channel is in the adapter's ``supported_channels``).
    The ``source_round`` is anchored at the round index so the ledger
    rows record the round progression.
    """
    return adapter.build_initial_state(
        batch_id="batch-stress-1000", sample_id="sample-stress-1000"
    )


def _drive_1000_rounds(*, seed: int = 42) -> list[EngineRoundResult]:
    ...
    for r in range(NUM_ROUNDS):
        ...
        # minted by the adapter. This matches the ``test_toy_gaussian``
        # pattern and produces empty ``audit_codes`` because the bundle
        # is structurally valid and the ``"x"`` channel is supported.
...
def _ledger_bytes(rows: Iterable[LedgerRow]) -> int:
...
# ---------------------------------------------------------------------------
# 1. Engine holds under 1000 rounds — empty audit, unique rows, budget.
# ---------------------------------------------------------------------------


@pytest.mark.stress
def test_engine_holds_under_1000_rounds() -> None:
    """Engine survives 1000 rounds with no audit codes, unique row IDs,
    bounded per-round wall-clock, and deterministic phase advance.
    ...
    * Phase state ``round_in_cycle`` is a strict counter and the
    ...
    phase_history: list[tuple[int, int]] = [
...
@pytest.mark.stress
def test_ledger_size_grows_linearly() -> None:
...
@pytest.mark.stress
def test_engine_byte_deterministic_under_long_horizon() -> None:
    """Two 1000-round sequences produce byte-identical final-state hashes.
...
def test_byte_determinism_holds_for_any_seed(seed: int) -> None:
    """For any ``seed`` in [0, 10_000], two 1000-round runs produce the
    same final-state hash.

    This is the property-based companion to
    :func:`test_engine_byte_deterministic_under_long_horizon` — the
    fixed-seed test verifies the canonical example; this one
...
__all__ = [
    "NUM_ROUNDS",
    "PER_ROUND_BUDGET_US",
    "test_engine_holds_under_1000_rounds",
    "test_ledger_size_grows_linearly",
    "test_engine_byte_deterministic_under_long_horizon",
    "test_byte_determinism_holds_for_any_seed",
]
