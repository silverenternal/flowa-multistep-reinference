...
Exercises :class:`adaptive_reflow.adapters.wan2_2_video_flowmatchingodeadapter
.Wan22VideoFlowMatchingODEAdapter` end-to-end. The adapter wraps a
...
@pytest.fixture
def wan22_adapter() -> object:  # Wan22VideoFlowMatchingODEAdapter
    """Return a fresh :class:`Wan22VideoFlowMatchingODEAdapter` (synthetic mode)."""
    from adaptive_reflow.adapters.wan2_2_video_flowmatchingodeadapter import (
        Wan22VideoFlowMatchingODEAdapter,
    )

    return Wan22VideoFlowMatchingODEAdapter(
        variant="t2v_a14b",
...
@pytest.fixture
def wan22_adapter_ti2v() -> object:  # Wan22VideoFlowMatchingODEAdapter (TI2V-5B)
    """Return a fresh :class:`Wan22VideoFlowMatchingODEAdapter` in TI2V-5B mode."""
    from adaptive_reflow.adapters.wan2_2_video_flowmatchingodeadapter import (
        Wan22VideoFlowMatchingODEAdapter,
    )
...
def test_velocity_field_lazy_loads_torch_if_required() -> None:
    ...
    from adaptive_reflow.adapters import wan2_2_video_flowmatchingodeadapter as wan22
    ...
    adapter = wan22.Wan22VideoFlowMatchingODEAdapter(
        variant="t2v_a14b",
        force_mode="synthetic",
        num_steps=3,
        solver="heun",
