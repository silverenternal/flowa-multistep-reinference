"""Comprehensive test suite for the FlowMol3AdapterImpl (DTB-G2 mechanics-adapter).

The :class:`adaptive_reflow.adapters.flowmol3adapter.FlowMol3AdapterImpl`
...
from adaptive_reflow.adapters.flowmol3adapter import (
...
@pytest.fixture
def adapter() -> FlowMol3AdapterImpl:
    """Yield a NumPy-backed :class:`FlowMol3AdapterImpl` for tests."""
    return FlowMol3AdapterImpl(backend="numpy", num_steps=10)


@pytest.fixture
def initial_bundle(adapter: FlowMol3AdapterImpl) -> StateBundle:
...
def test_capabilities_handshake() -> None:
    ...
    caps = FlowMol3AdapterCapabilities()
...
def test_build_initial_state_returns_correct_shape(
    adapter: FlowMol3AdapterImpl, initial_bundle: StateBundle
) -> None:
...
def test_determinism() -> None:
    ...
    a1 = FlowMol3AdapterImpl(backend="numpy", num_steps=10)
    a2 = FlowMol3AdapterImpl(backend="numpy", num_steps=10)
...
def test_velocity_field_lazy_loads_torch_if_required() -> None:
    ...
    a = FlowMol3AdapterImpl(backend="numpy", num_steps=5)
    ...
    from adaptive_reflow.adapters import flowmol3adapter as fma
    ...
    try:
        with pytest.raises(ImportError, match="torch backend requested"):
            FlowMol3AdapterImpl(backend="torch", num_steps=5)
    finally:
...
def test_default_factory() -> None:
    ...
    assert isinstance(a, FlowMol3AdapterImpl)
