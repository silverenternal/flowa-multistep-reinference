...
## S8. Appendix figures (Wave 143 Phase 3)

The following 9 appendix figures (A1, A1b, A2-A8) are qualitative
samples + 3D trajectories + convergence plots that supplement the
main-paper figures (fig 1-7 in `docs/figures/`). All nine are sourced
from existing verification outputs and headline-evidence directories;
no new experiments were run for this appendix. Generator:
`tools/_make_wave143_appendix_figures.py`.

| Figure | Description | Source data |
|---|---|---|
| A1   | Kanzi framework_inv_proj N=20 per-record RMSD trajectory | `verification_outputs/kanzi_inv_proj_n20_20260913_corrected/initial_summary.json` |
| A1b  | Kanzi seq_0 64-position codebook trajectory (3D scatter) | `verification_outputs/kanzi_inv_proj_n20_20260913_corrected/checkpoint.json` |
| A2   | 2D Two Moons qualitative samples (ground truth + baseline + framework) | `data/twodim_fm_two_moons.npz` + `adaptive_reflow/data/target_distributions.py` |
| A3   | 2D Eight Gaussians qualitative samples (ground truth + baseline + framework) | `data/twodim_fm_eight_gaussians.npz` + `adaptive_reflow/data/target_distributions.py` |
| A4   | CIFAR-10 v2 vs v4 FID comparison (4 versions, 1 chart) | `docs/CONSOLIDATED_RESULTS.md` §6 |
| A5   | 6 R* headline-evidence bars with Bonferroni-corrected p-values | `docs/headline-evidence/r{1..6}/*/SOURCE.md` |
| A6   | Per-cell p-value distribution across 12 measurement cells | `verification_outputs/power_analysis/per_cell.csv` |
| A7   | LineageFlow HMMER +116% headline bar (R1) | `docs/headline-evidence/r1_lineageflow_hmmer_p1e-10/SOURCE.md` |
| A8   | 3 composite axis byte-stable improvements (3/3 Tier 3 models) | `docs/headline-evidence/composite_axis_byte_stable/SOURCE.md` |

All nine figures are referenced in the appendix section of the
manifest at `docs/figures/README.md` (the "Appendix figures (Wave 143
Phase 3)" section). The figure assets are stored as PNG files at
`docs/figures/figA{1,1b,2..8}_*.png` and can be regenerated by
running `python3 tools/_make_wave143_appendix_figures.py`.
